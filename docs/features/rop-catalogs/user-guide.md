...
Для каждой активной выдачи дополнительно показывается, к какому **виду изделия** относится каталог.

![Карточка клиента, вкладка «Каталоги»: список активных выдач, «Выдать каталоги», «Вернуть» и «Печать» в строках](../../.gitbook/assets/1-catalogs-tab.png)
...
4. В блоке `Позиции` указать каталог и **количество** экземпляров (1–99) для каждой строки. В списке выбора каталог отображается вместе с видом изделия, чтобы было понятно, к какой группе он относится. Нужно несколько наименований или несколько экземпляров одной позиции — нажать `Добавить позицию` и заполнить ещё одну строку (либо повысить число в поле количества в существующей строке для одной передачи одного названия).
...
![Окно выдачи: позиции (каталог и количество), дата выдачи, примечание](../../.gitbook/assets/2-issue-catalogs-modal.png)
...
![Окно фиксации возврата: дата возврата и «Зафиксировать возврат»](../../.gitbook/assets/3-return-catalog-modal.png)
...
Также можно повторно открыть бланк: в строке любого активного каталога из этой передачи нажать `Печать`. Если в передаче было несколько каталогов, откроется общий бланк всей этой передачи.
...
* таблица: без повторного перечисления одного названия; для каждой позиции указываются каталог, **вид изделия** и количество (`Кол-во`);
* подписи `Передал` и `Принял`.

![Бланк передачи каталогов перед печатью](../../.gitbook/assets/4-transfer-blank.png)
...
![Раздел «Каталоги», вкладка «Аналитика»: показатели Выдано / Возвращено / Активно, группировка, фильтры и таблица](../../.gitbook/assets/5-analytics-tab.png)
